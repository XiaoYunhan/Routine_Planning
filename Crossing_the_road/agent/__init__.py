...
class GeneratePDDL_Stationary :
    ...
    def generateTimeStamp(self): #fix
    ...
    def generateInitString(self) :
        ...
        '''
        initString = "(at pt{}pt{} agent1)(at_time t{} agent1)".format(self.state.agent.position.x, self.state.agent.position.y, 0)
        ...
        for w in range(1, self.width):
            for lane in range(self.num_lanes):
                    GridRelativePosition = GridRelativePosition + "(forward_next pt{}pt{} pt{}pt{})".format(w, lane, w - 1, lane)
                    GridRelativePosition = GridRelativePosition + "(up_next pt{}pt{} pt{}pt{})".format(w, lane, w - 1, max(lane - 1, 0))
                    GridRelativePosition = GridRelativePosition + "(down_next pt{}pt{} pt{}pt{})".format(w, lane, w - 1, min(lane + 1, self.num_lanes - 1))

        for time in range(self.width - 1):
            TimeString = TimeString + "(time_next t{} t{})".format(time, time + 1)

        for time in range(self.width):
            for car in self.state.cars:
                position_x = car.position.x - abs(car.sample_speed()) * time
                while position_x < 0:
                    position_x = position_x + self.width
                for speed in range(abs(car.sample_speed())):
                    behind = position_x - speed
                    behind = behind - 1
                    while behind < 0:
                        behind = position_x + self.width
                    CarString = CarString + "(blocked t{} pt{}pt{})".format(time, behind, car.position.y)
                # currentBlockString = "(blocked t{} pt{}pt{})".format(time, position_x, car.position.y)
                # initString = initString + currentBlockString

        return initString + GridRelativePosition + TimeString + CarString
...
def generateDomainPDDLFile(gen):
    ...
    (blocked ?t1 ?pt) : The gridcell pt is occupied by a car and is "blocked" at t1.
    '''
    gen.addPredicate(name="at", parameters=(("pt1" , "gridcell"), ("agent", "agent")))
    gen.addPredicate(name="at_time", parameters=(("t1", "time"), ("agent", "agent")))
    ...
    gen.addPredicate(name="blocked", parameters=(("t1", "time"), ("pt1" , "gridcell")) , isLastPredicate=True)

    gen.addAction(name="UP",
                  parameters = (("pt1", "gridcell"), ("pt2", "gridcell"), ("agent", "agent"), ("t1", "time"), ("t2", "time")),
                  precondition_string = "(and (at ?pt1 ?agent) (up_next ?pt1 ?pt2) (not (blocked ?t1 ?pt2))) (at_time ?t1 ?agent) (time_next ?t1 ?t2)",
                  effect_string= "(and (at ?pt2 ?agent) (at_time ?t2 ?agent) (not(at ?pt1 ?agent)) (not (at_time ?t1 ?agent)))")

    gen.addAction(name="DOWN",
                  parameters = (("pt1", "gridcell"), ("pt2", "gridcell"), ("agent", "agent"), ("t1", "time"), ("t2", "time")),
                  precondition_string = "(and (at ?pt1 ?agent) (down_next ?pt1 ?pt2) (not (blocked ?t1 ?pt2)) (at_time ?t1 ?agent) (time_next ?t1 ?t2))",
                  effect_string= "(and (at ?pt2 ?agent) (at_time ?t2 ?agent) (not(at ?pt1 ?agent)) (not (at_time ?t1 ?agent)))")

    gen.addAction(name="FORWARD_1",
                  parameters = (("pt1", "gridcell"), ("pt2", "gridcell"), ("agent", "agent"), ("t1", "time"), ("t2", "time")),
                  precondition_string = "(and (at ?pt1 ?agent) (forward_next ?pt1 ?pt2) (not (blocked ?t1 ?pt2)) (at_time ?t1 ?agent) (time_next ?t1 ?t2))",
                  effect_string= "(and (at ?pt2 ?agent) (at_time ?t2 ?agent) (not(at ?pt1 ?agent)) (not (at_time ?t1 ?agent)))")

    gen.addAction(name="FORWARD_2",
                  parameters = (("pt1", "gridcell"), ("pt2", "gridcell"), ("pt3", "gridcell"), ("agent", "agent"), ("t1", "time"), ("t2", "time")),
                  precondition_string = "(and (at ?pt1 ?agent) (at_time ?t1 ?agent) (forward_next ?pt2 ?pt3) (not (blocked ?t1 ?pt2)) (not (blocked ?t1 ?pt3)) (time_next ?t1 ?t2) (forward_next ?pt1 ?pt2))",
                  effect_string= "(and (at ?pt3 ?agent) (at_time ?t2 ?agent) (not(at ?pt1 ?agent)) (not (at_time ?t1 ?agent)))")

    gen.addAction(name="FORWARD_3",
                  parameters = (("pt1", "gridcell"), ("pt2", "gridcell"), ("pt3", "gridcell"), ("pt4", "gridcell"), ("agent", "agent"), ("t1", "time"), ("t2", "time")),
                  precondition_string = "(and (at ?pt1 ?agent) (at_time ?t1 ?agent) (forward_next ?pt1 ?pt2) (forward_next ?pt2 ?pt3) (forward_next ?pt3 ?pt4) (not (blocked ?t1 ?pt2)) (not (blocked ?t1 ?pt3)) (not (blocked ?t1 ?pt4)) (time_next ?t1 ?t2))",
                  effect_string= "(and (at ?pt4 ?agent) (at_time ?t2 ?agent) (not(at ?pt1 ?agent)) (not (at_time ?t1 ?agent)))")
