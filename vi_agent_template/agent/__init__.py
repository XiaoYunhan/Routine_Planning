...
def value_iteration(trn_fn, rwd_fn, gamma):
    ...
    '''
    # print(matmul(trn_fn[0][0], rwd_fn[0]))
    value_q = np.zeros(rwd_fn.shape[0])  
    while True:
        diff = 0
        value_fn = value_q
        for s in range(len(rwd_fn)):
            action_list = []
            for a in range(len(trn_fn)):
                # action_sum = 0
                # for sq in range(len(trn_fn[0])):
                #     action_sum += gamma*trn_fn[a,s,sq]*value_fn[sq] + rwd_fn[s,sq]
                action_sum = gamma*matmul(trn_fn[a][s], (value_fn + rwd_fn[s]))
                action_list.append(action_sum)
            value_q[s] = max(action_list)
            policy[s] = action_list.index(value_q[s])
            if abs(value_q[s] - value_fn[s]) > diff:
                diff = abs(value_q[s] - value_fn[s])
        if diff < delta:
        # if diff < delta*(1-gamma)/gamma :
            break
